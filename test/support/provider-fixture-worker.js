...
export default {
  ...
  async fetch(request) {
    ...
    if (request.method === "GET" && url.origin === "https://graph.threads.net" && url.pathname === "/v1.0/debug_token" && exactQuery(url, ["input_token"]) && bearer(request) && request.headers.get("authorization") === `Bearer ${url.searchParams.get("input_token")}`) { console.log("provider_fixture:threads_debug_token"); return Response.json({ data: { app_id: "test-threads-app", user_id: "author-1", is_valid: true, expires_at: 2_000_000_000, scopes: ["threads_basic", "threads_profile_discovery", "threads_read_replies"] } }); }
    ...
    if (request.method === "GET" && url.origin === "https://graph.threads.net" && url.pathname === "/v1.0/profile_posts" && exactQuery(url, url.searchParams.has("after") ? ["fields", "username", "after"] : ["fields", "username"]) && url.searchParams.get("fields") === fields && url.searchParams.get("username") === "meta" && (!url.searchParams.has("after") || url.searchParams.get("after") === "cursor-1") && bearer(request)) { console.log("provider_fixture:threads_profile_posts"); return Response.json({ data: url.searchParams.has("after") ? [] : [root] }); }
    ...
    if (request.method === "GET" && url.origin === "https://graph.threads.net" && url.pathname === "/v1.0/root-1/conversation" && exactQuery(url, url.searchParams.has("after") ? ["fields", "after"] : ["fields"]) && url.searchParams.get("fields") === fields && (!url.searchParams.has("after") || url.searchParams.get("after") === "cursor-1") && bearer(request)) { console.log("provider_fixture:threads_conversation"); return Response.json({ data: [] }); }
