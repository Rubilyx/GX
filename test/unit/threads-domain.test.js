...
test("normalizes canonical and legacy Threads URLs", () => {
  ...
  for (const x of ["https://threads.com.evil/t/x","https://u:p@threads.com/t/x","https://threads.com:443/t/x","https://threads.com/t/x/y","https://threads.com/t/%ZZ"]) invalid(() => normalizeThreadsUrl(x), "invalid_threads_url");
});
...
test("parses strict Threads query pages", () => {
  ...
  for (const [fn, key] of [[parseThreadsListQuery,"page"],[parseThreadsDetailQuery,"repliesPage"]]) { invalid(() => fn(new URL(`https://x/?${key}=1&${key}=2`)), "invalid_threads_query"); invalid(() => fn(new URL("https://x/?other=1")), "invalid_threads_query"); }
});
